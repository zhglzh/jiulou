...
TreeGrid = function(_config){
	...
	drowRowData = function(_rows, _cols, _level, _pid){
		...
		for(var i=0;i<_rows.length;i++){
			...
			var dispaly = "none";
			if(_level<=expandLayer+1) dispaly = ""; //显示层等于展开层+1
			
			s += "<tr id='TR" + id + "' pid='" + ((_pid=="")?"":("TR"+_pid)) + "' open='" + open + "' data=\"" + TreeGrid.json2str(row) + "\" rowIndex='" + rownum++ + "' style='display:" + dispaly + "'>";
			for(var j=0;j<_cols.length;j++){
				...
				if(j==folderColumnIndex){
					if(row.children){ //有下级数据
						if(open=="Y"){
							s += "<img style=\"width: 16px; height: 16px; margin-bottom: -4px;\" folder='Y' trid='TR" + id + "' src='" + folderOpenIcon + "' class='image_hand'>";
						}else{
							s += "<img style=\"width: 16px; height: 16px; margin-bottom: -4px;\" folder='Y' trid='TR" + id + "' src='" + folderCloseIcon + "' class='image_hand'>";
						}
					}else{
						s += "<img style=\"width: 16px; height: 16px; margin-bottom: -4px;\" src='" + defaultLeafIcon + "' class='image_nohand'>";
					}
				}
...
function TreeGridItem (_root, _rowId, _rowIndex, _rowData){
	...
	this.getParent = function(){
		var pid = jQuery("#" + this.id).prop("pid");
		if(pid!=""){
			var rowIndex = jQuery("#" + pid).prop("rowIndex");
			var data = jQuery("#" + pid).prop("data");
